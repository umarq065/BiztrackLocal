      ...
      {showChart && growthMetricsData?.timeSeries && currentPeriodTimeSeries && (
        <CardContent className="space-y-6">
          <Suspense fallback={<Skeleton className="h-[400px] w-full" />}>
            <GrowthMetricsChart
              timeSeries={[
                ...(growthMetricsData?.timeSeries?.previousPeriod || []).map(d => ({ date: d.date, value: d.revenue, note: d.note })),
                ...currentPeriodTimeSeries.map(d => ({ date: d.date, value: d.revenue, note: d.note }))
              ]}
              dateRange={date}
            />
          </Suspense>
